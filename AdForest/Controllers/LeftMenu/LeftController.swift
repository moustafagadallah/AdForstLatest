...
class LeftController: UIViewController, UITableViewDelegate, UITableViewDataSource, NVActivityIndicatorViewable , changeOtherMenuesProtocol , guestMenuProtocol, changeOtherGuestProtocol, changePagesProtocol, leftMenuProtocol,leftMenuLangProtocol {
    ...
    func handleSideMenuItems()  {
        if defaults.bool(forKey: "isLogin") == true {
            
            if let index = UserHandler.sharedInstance.menuValuesArray.firstIndex(of: "إعلانات غير نشطة") {
                UserHandler.sharedInstance.menuValuesArray.remove(at: index)
            }
            if let index = UserHandler.sharedInstance.menuValuesArray.firstIndex(of:"البائعون") {
                UserHandler.sharedInstance.menuValuesArray.remove(at: index)
            }
            if let index = UserHandler.sharedInstance.menuValuesArray.firstIndex(of:"الإعلانات المميزة") {
                UserHandler.sharedInstance.menuValuesArray.remove(at: index)
            }
            
            if let index = UserHandler.sharedInstance.menuValuesArray.firstIndex(of:"الإعلانات المرفوضة") {
                UserHandler.sharedInstance.menuValuesArray.remove(at: index)
            }
            if let index = UserHandler.sharedInstance.menuValuesArray.firstIndex(of:"الإعلانات المفضلة") {
                UserHandler.sharedInstance.menuValuesArray.remove(at: index)
            }
            if let index = UserHandler.sharedInstance.menuValuesArray.firstIndex(of:"الإعلانات منتهية الصلاحية/المباعة") {
                UserHandler.sharedInstance.menuValuesArray.remove(at: index)
            }
            if let index = UserHandler.sharedInstance.menuValuesArray.firstIndex(of:"اعلاناتي")  {
                UserHandler.sharedInstance.menuValuesArray.remove(at: index)
            }
            ///////////////////////
            if let index = UserHandler.sharedInstance.menuValuesArray.firstIndex(of: "Inactive Ads") {
                UserHandler.sharedInstance.menuValuesArray.remove(at: index)
            }
            if let index = UserHandler.sharedInstance.menuValuesArray.firstIndex(of:"Sellers") {
                UserHandler.sharedInstance.menuValuesArray.remove(at: index)
            }
            if let index = UserHandler.sharedInstance.menuValuesArray.firstIndex(of:"Featured Ads") {
                UserHandler.sharedInstance.menuValuesArray.remove(at: index)
            }
            
            if let index = UserHandler.sharedInstance.menuValuesArray.firstIndex(of:"Rejected Ads") {
                UserHandler.sharedInstance.menuValuesArray.remove(at: index)
            }
            if let index = UserHandler.sharedInstance.menuValuesArray.firstIndex(of:"Fav Ads") {
                UserHandler.sharedInstance.menuValuesArray.remove(at: index)
            }
            if let index = UserHandler.sharedInstance.menuValuesArray.firstIndex(of:"Expired/Sold  Ads") {
                UserHandler.sharedInstance.menuValuesArray.remove(at: index)
            }
            if let index = UserHandler.sharedInstance.menuValuesArray.firstIndex(of:"My Ads")  {
                UserHandler.sharedInstance.menuValuesArray.remove(at: index)
            }
        }
    }
